...
def classify_images(images_dir, results_dic, model):
    ...
    for filename, labels in results_dic.items():
        ...
        # Use the classifier function to get the label from the model
        classifier_label = classifier(image_path, model)
        
        # Format the classifier label: lowercase, strip whitespace
        classifier_label = classifier_label.lower().strip()
        
        # Compare pet label with classifier label and check for a match
        pet_label = labels[0]
        match = 1 if pet_label in classifier_label.split(", ") else 0
